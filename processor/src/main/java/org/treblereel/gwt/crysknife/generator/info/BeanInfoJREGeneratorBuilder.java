...
public class BeanInfoJREGeneratorBuilder extends AbstractBeanInfoGenerator {
    ...
    private void addFields() {
        for (FieldPoint fieldPoint : bean.getFieldInjectionPoints()) {
            ...
            MethodDeclaration methodDeclaration = classBuilder.addMethod(fieldPoint.getName(), Modifier.Keyword.PUBLIC);
            ...
            methodDeclaration.getBody().ifPresent(body -> {
                body.addAndGetStatement(new ReturnStmt(new MethodCallExpr("onInvoke")
                                                               ...
                                                               .addArgument(new StringLiteralExpr(fieldPoint.getName()))
    ...
    private void generateFactoryFieldDeclaration(FieldPoint fieldPoint) {
        ...
        type.setTypeArguments(new ClassOrInterfaceType().setName(fieldPoint.getType().getQualifiedName().toString()));
        ...
        if (fieldPoint.isNamed()) {
            typeElement = iocContext.getQualifiers().get(fieldPoint.getType()).get(fieldPoint.getNamed()).getType();
        } else {
        ...
        classBuilder.addFieldWithInitializer(supplier, fieldPoint.getName(), lambda, Modifier.Keyword.PRIVATE);
    }
