...
import org.treblereel.gwt.crysknife.client.internal.collections.BiMap;
...
import org.treblereel.gwt.crysknife.navigation.client.local.URLPattern;
import org.treblereel.gwt.crysknife.navigation.client.local.URLPatternMatcher;
import org.treblereel.gwt.crysknife.navigation.client.local.api.NavigationControl;
import org.treblereel.gwt.crysknife.navigation.client.local.api.PageRequest;
...
public class NavigationGraphGenerator {
    ...
    public String generateSource(final GenerationContext context, final String fqcn) {
        ...
        ClassOrInterfaceDeclaration classDeclaration = compilationUnit.addClass(CLASS_NAME).setPublic(true);
    ...
    private void generatePage(TypeElement page, ConstructorDeclaration ctor) {
        if (!isAssignable(page.asType(), IsElement.class)) {
            throw new GenerationException("Class " + page
                                                  + " is annotated with @Page, so it must implement IsElement");
        }
        ...
        if (prevPageWithThisName != null) {
            throw new GenerationException(
                    "Page names must be unique, but " + prevPageWithThisName + " and " + page +
                            " are both named [" + pageName + "]");
        }

        String pageImplStmt = generateNewInstanceOfPageImpl(page, pageName, ctor);

        TypeMirror defaultPage = context.getElements().getTypeElement(DefaultPage.class.getCanonicalName()).asType();
    ...
    private String generateNewInstanceOfPageImpl(TypeElement page, String pageName, ConstructorDeclaration constructorDeclaration) {
        NodeList<BodyDeclaration<?>> anonymousClassBody = new NodeList<>();
        String instanceName = toLowerCase(page);

        ClassOrInterfaceType type = new ClassOrInterfaceType().setName("PageNode<" + page.getSimpleName().toString() + ">");
    ...
    private String toLowerCase(TypeElement page) {
        return Character.toLowerCase(page.getSimpleName().toString().charAt(0)) + page.getSimpleName().toString().substring(1);
    }
    ...
    private void generateGetURL(TypeElement page, String pageName, NodeList<BodyDeclaration<?>> anonymousClassBody) {
        MethodDeclaration method = new MethodDeclaration();
        method.setModifiers(Modifier.Keyword.PUBLIC);
        method.setName("getURL");
        method.setType(new ClassOrInterfaceType().setName("String"));
        method.getBody().get().addAndGetStatement(new ReturnStmt(new StringLiteralExpr(getPageURL(page, pageName))));
        anonymousClassBody.add(method);
    }

    private void generateContentType(TypeElement page, String pageName, NodeList<BodyDeclaration<?>> anonymousClassBody) {
        MethodDeclaration method = new MethodDeclaration();
        method.setModifiers(Modifier.Keyword.PUBLIC);
        method.setName("contentType");
        method.setType(new ClassOrInterfaceType().setName("Class"));
        method.getBody().get().addAndGetStatement(new ReturnStmt(new NameExpr(page.getQualifiedName() + ".class")));
        anonymousClassBody.add(method);
    }

    private void generateProduceContent(TypeElement page, String pageName, NodeList<BodyDeclaration<?>> anonymousClassBody) {
        MethodDeclaration method = new MethodDeclaration();
        ...
        method.getBody().get().addAndGetStatement(
                ...
                new MethodCallExpr(
                        new NameExpr("callback"), "callback")
    ...
    private void generatePageHiding(TypeElement page, String pageName, NodeList<BodyDeclaration<?>> anonymousClassBody) {
        MethodDeclaration method = new MethodDeclaration();
        ...
        if (executableElement != null) {
            method.getBody().get().addAndGetStatement(new MethodCallExpr(new NameExpr("widget"), executableElement.getSimpleName().toString()));
        }
        method.getBody().get().addAndGetStatement(new MethodCallExpr(new NameExpr("control"), "proceed"));
        anonymousClassBody.add(method);
    }

    private void generatePageHidden(TypeElement page, String pageName, NodeList<BodyDeclaration<?>> anonymousClassBody) {
        MethodDeclaration method = new MethodDeclaration();
        method.addParameter(page.getSimpleName().toString(), "widget");
        method.setModifiers(Modifier.Keyword.PUBLIC);
        method.setType("void");
    ...
    private void appendPageShowingMethod(TypeElement page, String pageName, NodeList<BodyDeclaration<?>> anonymousClassBody) {
        MethodDeclaration method = new MethodDeclaration();
        method.setModifiers(Modifier.Keyword.PUBLIC);
        ...
        ObjectCreationExpr mapCreationExpr = new ObjectCreationExpr().setType(new ClassOrInterfaceType().setName(HashMap.class.getSimpleName()));

        method.getBody().get().addAndGetStatement(new AssignExpr().setTarget(
                new VariableDeclarationExpr(
                        new ClassOrInterfaceType().setName(Map.class.getSimpleName()), "pageState"))
                                                          .setValue(mapCreationExpr));
        ObjectCreationExpr pageRequest = new ObjectCreationExpr().setType(
                new ClassOrInterfaceType().setName(PageRequest.class.getSimpleName()))
                .addArgument(new StringLiteralExpr(pageName))
                .addArgument("pageState");

        ObjectCreationExpr navigationEvent = new ObjectCreationExpr().setType(
                new ClassOrInterfaceType().setName(NavigationEvent.class.getSimpleName()))
                .addArgument(pageRequest);

        method.getBody().get().addAndGetStatement(
                new MethodCallExpr(
                        new NameExpr("event"), "fire")
                        .addArgument(navigationEvent));
    ...
    private void appendPageShownMethod(TypeElement page, String pageName, NodeList<BodyDeclaration<?>> anonymousClassBody) {
        MethodDeclaration method = new MethodDeclaration();
        method.setModifiers(Modifier.Keyword.PUBLIC);
    ...
    private void appendPageUpdateMethod(TypeElement page, String pageName, NodeList<BodyDeclaration<?>> anonymousClassBody) {
        MethodDeclaration method = new MethodDeclaration();
        method.setModifiers(Modifier.Keyword.PUBLIC);
    ...
    private void appendDestroyMethod(TypeElement page, String pageName, NodeList<BodyDeclaration<?>> anonymousClassBody) {
        MethodDeclaration method = new MethodDeclaration();
        method.addParameter(page.getSimpleName().toString(), "widget");
        method.setModifiers(Modifier.Keyword.PUBLIC);
        ...
        method.getBody().get().addAndGetStatement(new MethodCallExpr(new NameExpr("beanManager"), "destroyBean").addArgument("widget"));
        anonymousClassBody.add(method);
    }
    ...
    private ExecutableElement checkMethod(TypeElement pageClass, Class<? extends Annotation> annotation) {
        List<ExecutableElement> annotatedMethods = pageClass.getEnclosedElements()
                .stream()
                .filter(elm -> elm.getKind().equals(ElementKind.METHOD))
                .map(elm -> MoreElements.asExecutable(elm))
                .filter(elm -> elm.getAnnotation(annotation) != null)
                .collect(Collectors.toList());

        if (annotatedMethods.size() > 1) {
            throw new UnsupportedOperationException(
                    "A @Page can have at most 1 " + createAnnotationName(annotation) + " method, but " + pageClass + " has "
        ...
        if (annotatedMethods.size() == 1) {
            ...
            if (!method.getReturnType().getKind().equals(TypeKind.VOID)) {
                throw new UnsupportedOperationException(
                        createAnnotationName(annotation) + " methods must have a void return type, but " +
            ...
            if (!method.getModifiers().contains(javax.lang.model.element.Modifier.PUBLIC)) {
                throw new UnsupportedOperationException("Method " + method.getEnclosingElement() + "." + method.getSimpleName() + " annotated with " +
                                                                createAnnotationName(annotation) + " must be public ");
            }
