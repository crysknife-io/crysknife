...
public class TransitionToGenerator extends ScopedBeanGenerator {
    ...
    public Expression generateBeanCall(ClassBuilder clazz, FieldPoint fieldPoint, BeanDefinition beanDefinition) {
        clazz.getClassCompilationUnit().addImport("org.treblereel.gwt.crysknife.navigation.client.local.TransitionTo");

        return new ObjectCreationExpr().setType(TransitionTo.class)
                .addArgument(MoreTypes.asDeclared(fieldPoint.getField()
                                                          .asType()).getTypeArguments().get(0).toString() + ".class")
                .addArgument(new MethodCallExpr(new MethodCallExpr(new MethodCallExpr(new NameExpr("BeanManagerImpl"), "get"), "lookupBean")
                                                        .addArgument(Navigation.class.getCanonicalName() + ".class"), "get"));
    }
}
