...
package io.crysknife.ui.validation.client.utils;
...
public class StringFormat {
  ...
  /**
   * This method emulates {@link String#format(String, Object...)} with some notable differences:
   * <ul>
   * <li>flags are unsupported (and are silently ignored)</li>
   * <li>the '{@code a}' and '{@code A}' conversions are unsupported</li>
   * <li>the other scientific notation flags use {@link NumberFormat} and thus produce slightly
   * different output</li>
   * </ul>
   * 
   * @param format The format string.
   * @param args The values available for format string conversions.
   * @return A formatted string, similar to the result of calling
   *         {@link String#format(String, Object...)} with {@code format} and {@code args}.
      ...
      if (format.charAt(i) == '%') {
        ...
        if (match.getGroup(2) != null && !match.getGroup(2).equals("")) {
          throw new UnsupportedOperationException(
              "Flags are not yet supported in this implementation.");
        }
  ...
  private static String buildReplacement(Conversion conv, boolean upper, int width, int prec,
      String suffix, Object arg) {
    String res = null;
    switch (conv) {
      case bool:
  ...
  private static String processDate(Date date, boolean upper, char suffix) {
    ...
    switch (suffix) {
      ...
      case 'p':
        retVal =
            LocaleInfo.getCurrentLocale().getDateTimeFormatInfo().ampms()[date.getHours() / 12];
        break;
      case 's':
      ...
      case 'B':
        retVal =
            LocaleInfo.getCurrentLocale().getDateTimeFormatInfo().monthsFull()[date.getMonth()];
        break;
      case 'b':
      case 'h':
        retVal =
            LocaleInfo.getCurrentLocale().getDateTimeFormatInfo().monthsShort()[date.getMonth()];
        break;
      case 'A':
        retVal =
            LocaleInfo.getCurrentLocale().getDateTimeFormatInfo().weekdaysFull()[date.getDay()];
        break;
      case 'a':
        retVal =
            LocaleInfo.getCurrentLocale().getDateTimeFormatInfo().weekdaysShort()[date.getDay()];
        break;
      case 'c':
