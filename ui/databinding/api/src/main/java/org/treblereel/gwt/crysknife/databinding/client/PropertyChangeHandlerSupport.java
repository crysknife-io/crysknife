...
public class PropertyChangeHandlerSupport {
   ...
   * final Multimap<String, PropertyChangeHandler<?>> removed =
   * ArrayListMultimap.create(specificPropertyHandlers); specificPropertyHandlers.clear();
   * 
   * return removed; }
   */
   ...
   *
   * @param handler The {@link PropertyChangeHandler} to add, must not be null.
   */
   ...
   *
   * @param name The property name for which notifications should be sent.
   * @param handler The {@link PropertyChangeHandler} to add, must not be null.
   */
  ...
  public void notifyHandlers(PropertyChangeEvent<?> event) {
    ...
    final Collection<PropertyChangeHandler<?>> specHandlers =
        specificPropertyHandlers.get(event.getPropertyName());
    for (PropertyChangeHandler handler : specHandlers
        .toArray(new PropertyChangeHandler<?>[specHandlers.size()])) {
      handler.onPropertyChange(event);
    }

    for (PropertyChangeHandler handler : handlers
        .toArray(new PropertyChangeHandler<?>[handlers.size()])) {
      handler.onPropertyChange(event);
    }
  }
