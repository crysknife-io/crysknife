...
public final class BindableProxyAgent<T> implements HasPropertyChangeHandlers {
     ...
     * @param bindOnKeyUp a flag indicating that the property should be updated when the widget fires a
     * {@link org.gwtproject.event.dom.client.KeyUpEvent} , in addition to the default
     ...
     * @param modelUpdater A {@link Consumer<?>} that updates the bound property of the model in response to UI changes.
    ...
    private static Supplier<Map<Class<? extends org.gwtproject.event.shared.Event>, HandlerRegistration>> addHandlers(
        ...
        Supplier<Map<Class<? extends org.gwtproject.event.shared.Event>, HandlerRegistration>> registrar = HashMap::new;
    ...
    private static void addChangeEventListener(Object element, JavaScriptObject listener) {
        ((HTMLElement) Js.uncheckedCast(element)).addEventListener("change", Js.uncheckedCast(listener));
    }
    ...
    private static void removeChangeEventListener(Object element, JavaScriptObject listener) {
        ((HTMLElement) Js.uncheckedCast(element)).removeEventListener("change", Js.uncheckedCast(listener));
    }
    ...
    private static Supplier<Map<Class<? extends org.gwtproject.event.shared.Event>, HandlerRegistration>> mergeHasValueChangeHandler(
        ...
        registrar = mergeToLeft(registrar, () -> {
            ...
            final HandlerRegistration valueHandlerReg = ((HasValue) component).addValueChangeHandler(event -> {
                ...
                DomGlobal.console.debug("   on value {" + component + "}" + ((HasValue) component).getValue());
     ...
     * @param converter the converter to use for this binding, null if default conversion should be used. See
     ...
     * @param component the UI component to bind to, must not be null.
     * @param property the property of the model to bind the widget to, must not be null.
     * @param providedConverter the converter to use for this binding, null if default conversion should be used. See
     * {@link Convert#getConverter(Class, Class)} and {@link Convert#identityConverter(Class)} for possible
     * arguments.
     * @param bindOnKeyUp a flag indicating that the property should be updated when the widget fires a
     * {@link KeyUpEvent} along with the default
     * {@link ValueChangeEvent}.
     * @param initialState specifies whether to use the model or component state when initially synchronizing this binding. If null,
    ...
    public Binding bind(final Object component, final String property, final Converter providedConverter,
        ...
        DomGlobal.console.debug("Using converter: {" + converter.getModelType().getSimpleName() + "} <--> {" + converter.getComponentType().getSimpleName() + "}");
    ...
    private void trySettingModelProperty(final String property, final Converter converter, final Object uiValue,
        ...
        try {
            ...
            if (newValue == null && isCausedByNullOrUndefined(t)) {
                 ...
                 */
                DomGlobal.console.warn("Encountered a null while trying to set the property [" + property + "] to ["
                                               + newValue + "].", t);
            } else {
            ...
            if (converter == null) {
                throw new RuntimeException(
                        "Cannot convert between " + propertyType.getName() + " and " + effectiveComponentType.getName()
                                + " for property [" + property + "] in " + proxy.getClass().getName());
            }
     ...
     * @param sync True if a {@link BindableListChangeHandler} component bound to a list should have it's value set via
     ...
     * @param oldValue The old value of the property.
     * @param newValue The new value of the property.
     * @param excluding A widget reference that does not need to be updated (the origin of
        ...
        for (final Binding binding : bindings.get(property)) {
            ...
            if (component instanceof TakesValue) {
                ...
                updateComponentValue(newValue, (HasText) component, converter);
                    ...
                    || component instanceof IsElement || isElement(component)) {
     ...
     * @param uiGetter An optional function for getting the value from the UI component. Must not be null.
     * @param initialState If {@link StateSync#FROM_MODEL} the UI value will be updated from the model. If {@link StateSync#FROM_UI}
     ...
     * @param pchs the instance who's change handlers will be merged, must not be
     * null.
     */
     ...
     * @param initialState If {@link StateSync#FROM_MODEL} the state from this agent's model overrides the other. If
