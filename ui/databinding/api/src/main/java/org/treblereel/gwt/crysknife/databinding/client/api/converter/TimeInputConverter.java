...
package org.treblereel.gwt.crysknife.databinding.client.api.converter;
...
public class TimeInputConverter extends AbstractDateInputConverter {
  ...
  private static final RegExp timeRegex =
      RegExp.compile("(\\d{2}):(?:(\\d{2}))(?::(\\d{2})(?:.(\\d{3}))?)?");
  ...
  public Date toModelValue(final String widgetValue) {
    ...
    if (result == null) {
      ...
      final int hrs = Integer.valueOf(result.getGroup(1)),
  ...
  public String toWidgetValue(final Date modelValue) {
    ...
    final String hrs = asPaddedString(modelValue.getHours(), 2),
