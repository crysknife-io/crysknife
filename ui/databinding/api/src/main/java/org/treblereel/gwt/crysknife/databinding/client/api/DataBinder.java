...
import org.treblereel.gwt.crysknife.databinding.client.Binding;
import org.treblereel.gwt.crysknife.databinding.client.ComponentAlreadyBoundException;
import org.treblereel.gwt.crysknife.databinding.client.HasPropertyChangeHandlers;
import org.treblereel.gwt.crysknife.databinding.client.InvalidPropertyExpressionException;
import org.treblereel.gwt.crysknife.databinding.client.MapBindableProxy;
import org.treblereel.gwt.crysknife.databinding.client.NonExistingPropertyException;
import org.treblereel.gwt.crysknife.databinding.client.OneTimeUnsubscribeHandle;
import org.treblereel.gwt.crysknife.databinding.client.PropertyChangeHandlerSupport;
import org.treblereel.gwt.crysknife.databinding.client.PropertyChangeUnsubscribeHandle;
...
public class DataBinder<T> implements HasPropertyChangeHandlers {
  private final PropertyChangeHandlerSupport propertyChangeHandlerSupport =
      new PropertyChangeHandlerSupport();
   ...
   *
   * @param propertyTypes A collection of name-type pairs describing the properties that can be in
   *        map models for the returned binder. Adding other properties to the model of the returned
   *        binder will cause {@link NonExistingPropertyException NonExistingPropertyExceptions} to
   *        be thrown.
   */
  public static DataBinder<Map<String, Object>> forMap(
  ...
  /**
   * Binds the provided component to the specified property of the model instance associated with
   * this {@link DataBinder}. If the provided component already participates in another binding
   * managed by this {@link DataBinder}, a {@link ComponentAlreadyBoundException} will be thrown.
   *
   * @param component The UI component the model instance should be bound to, must not be null.
   ...
   *         invalid.
   * @throws ComponentAlreadyBoundException If the provided {@code component} is already bound to a
  ...
  public DataBinder<T> bind(final Object component, final String property,
    ...
    final Binding binding =
        getAgent().bind(component, property, converter, bindOnKeyUp, initialState);
  ...
  /**
   * Unbinds all widgets bound by previous calls to {@link #bind(HasValue, Object, String)} and all
   ...
   *         automatically synchronized with the UI. If this binder has been {@link #pause() paused}
   *         then the this returns the paused model that may not be synchronized with the UI.
   */
  public T getModel() {
    ensureProxied();
   ...
   * @return The model instance which has to be used in place of the provided model (see
   *         {@link #forModel(Object)} and {@link #forType(Class)}) if changes should be
   *         automatically synchronized with the UI. Unlike {@link #getModel()} this method always
   *         returns a model that is sychronized to the UI, even if this binder has been paused.
   */
   ...
   * with the UI. The bound UI widgets will be updated based on the new model state. Bindings will
   ...
   * Changes the underlying model instance. The existing bindings stay intact but only affect the
   * new model instance. The previously associated model instance will no longer be kept in sync
   * with the UI. Bindings will be resumed if they are currently paused.
   *
   * @param model The instance of a {@link Bindable} type, must not be null.
  ...
  public T setModel(final T model, final StateSync initialState, final boolean fireChangeEvents) {
    ...
    for (final Binding b : this.bindings.values()) {
      ...
      bindings.put(b.getProperty(), newProxy.getBindableProxyAgent().bind(b.getComponent(),
          b.getProperty(), b.getConverter(), bindOnKeyUp, newInitState));
    }
  ...
  /**
   * Pauses all bindings. The model and UI fields are no longer kept in sync until either
   * {@link #resume(StateSync)} or {@link #setModel(Object)} is called. This method has no effect if
   * the bindings are already paused.
   */
  @SuppressWarnings("unchecked")
  public void pause() {
    if (paused != null)
  ...
  /**
   * Resumes the previously paused bindings (see {@link #pause()}) and carries out state
   * synchronization to catch up on changes that happened in the meantime. This method has no effect
   * if {@link #pause()} was never called.
   *
   * @param resumeState the state to resume from. Must not be null.
   */
  ...
  /**
   * @return true iff {@link #pause()} was called and there have been no calls since to
   *         {@link #resume(StateSync)} or {@link #setModel(Object, StateSync, boolean)}.
   */
  ...
  public PropertyChangeUnsubscribeHandle addPropertyChangeHandler(
    ...
    final PropertyChangeUnsubscribeHandle agentUnsubHandle =
