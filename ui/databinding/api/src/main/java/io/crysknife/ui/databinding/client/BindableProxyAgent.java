 ...
 * (see {@link DataBinder#setModel(Object, StateSync)})</li>
 *
 * <li>Update the bound widget when a setter method is invoked on the model (see
 * {@link #updateWidgetsAndFireEvent(boolean, String, Object, Object)}). Works for components that
 * either implement {@link TakesValue} or {@link HasText}). Also works for native wrappers of input
 * elements. Components displaying lists can implement {@link BindableListChangeHandler} to receive
 * incremental updates of the model state.</li>
 *
 * <li>Update the bound components when a non-accessor method is invoked on the model (by comparing
 * all bound properties to detect changes). See {@link #updateWidgetsAndFireEvents()}. Works for
 * components that either implement {@link TakesValue} or {@link HasText})</li>
 *
 * <li>Update the target model in response to value change events (only works for bound components
 * that implement {@link HasValue})</li>
 * <ul>
 * 
 * @param <T> The type of the target model being proxied.
 * 
...
public final class BindableProxyAgent<T> implements HasPropertyChangeHandlers {
  ...
  private static PropertyType getPropertyType(final BindableProxy<?> proxy, final String property) {
    ...
    if (property.contains(".")) {
      ...
      if (topLevelType instanceof MapPropertyType) {
        topLevelProxy = (BindableProxy<?>) DataBinder
            .forMap(((MapPropertyType) topLevelType).getPropertyTypes()).getModel();
      } else {
  ...
  /**
   * Makes a supplier for adding the required event handlers to the provided component. A
   * {@link ValueChangeHandler} is added by default, a {@link KeyUpHandler} is added if bindOnKeyUp
   * is true.
   * 
   * @param component the bound UI component, must not be null.
   * @param bindOnKeyUp a flag indicating that the property should be updated when the widget fires
   *        a {@link org.gwtproject.event.dom.client.KeyUpEvent} , in addition to the default
   *        {@link org.gwtproject.event.logical.shared.ValueChangeEvent}.
   * @param uiGetter an optional function for extracting the UI component value.
   * @param modelUpdater A {@link Consumer<?>} that updates the bound property of the model in
   *        response to UI changes.
   * 
   * @return a supplier that registers handlers and returns a collection of event handler
   *         registrations.
   */
  private static Supplier<Map<Class<? extends org.gwtproject.event.shared.Event>, HandlerRegistration>> addHandlers(
    ...
    if (bindOnKeyUp) {
      if (component instanceof ValueBoxBase) {
        ...
        throw new RuntimeException(
            "Cannot bind component " + component.toString() + " on key up events, "
                + component.toString() + " is not a ValueBoxBase or an element wrapper");
      }
    }
  ...
  private static boolean isElement(final Object obj) {
    return obj instanceof JavaScriptObject && Node.is((JavaScriptObject) obj)
        && Element.is((Node) obj);
  }

  private static JavaScriptObject wrap(Runnable runnable) {
    FnWithoutArgs fn = () -> runnable.run();
    return Js.uncheckedCast(fn);
  }

  private static void addChangeEventListener(Object element, JavaScriptObject listener) {
    ((HTMLElement) Js.uncheckedCast(element)).addEventListener("change",
        Js.uncheckedCast(listener));
  }

  private static void addKeyUpEventListener(Object element, JavaScriptObject listener) {
    ((HTMLElement) Js.uncheckedCast(element)).addEventListener("keyup", Js.uncheckedCast(listener));
  }

  private static void removeChangeEventListener(Object element, JavaScriptObject listener) {
    ((HTMLElement) Js.uncheckedCast(element)).removeEventListener("change",
        Js.uncheckedCast(listener));
  }

  private static void removeKeyUpEventListener(Object element, JavaScriptObject listener) {
    ((HTMLElement) Js.uncheckedCast(element)).removeEventListener("change",
        Js.uncheckedCast(listener));
  }

  private static Supplier<Map<Class<? extends org.gwtproject.event.shared.Event>, HandlerRegistration>> mergeValueBoxKeyUpHandler(
  ...
  private static Supplier<Map<Class<? extends org.gwtproject.event.shared.Event>, HandlerRegistration>> mergeHasValueChangeHandler(
      final Object component, final Consumer<Object> modelUpdater,
      Supplier<Map<Class<? extends org.gwtproject.event.shared.Event>, HandlerRegistration>> registrar) {
  ...
  private static Supplier<Map<Class<? extends org.gwtproject.event.shared.Event>, HandlerRegistration>> mergeNativeChangeEventListener(
      final Object component, final Optional<Supplier<Object>> uiGetter,
      final Consumer<Object> modelUpdater,
      Supplier<Map<Class<? extends org.gwtproject.event.shared.Event>, HandlerRegistration>> registrar) {

    registrar = mergeToLeft(registrar, () -> {
      DomGlobal.console.debug("Adding native change event listener to {" + component + "}");
      final JavaScriptObject listener =
          wrap(() -> uiGetter.ifPresent(getter -> modelUpdater.accept(getter.get())));
  ...
  private static Supplier<Map<Class<? extends org.gwtproject.event.shared.Event>, HandlerRegistration>> mergeNativeKeyUpEventListener(
      final Object component, final Optional<Supplier<Object>> uiGetter,
      final Consumer<Object> modelUpdater,
      Supplier<Map<Class<? extends org.gwtproject.event.shared.Event>, HandlerRegistration>> registrar) {
    ...
    registrar = mergeToLeft(registrar, () -> {
      final JavaScriptObject listener = wrap(() -> {
        DomGlobal.console.debug("keyup listener invoked for {" + component + "}");
        uiGetter.ifPresent(getter -> {
          final Object value = getter.get();
          DomGlobal.console.debug("keyup listener invoked with UI value {" + value + "}");
          modelUpdater.accept(value);
        });
      });
      DomGlobal.console.debug("Adding native keyup listener to {" + component + "}...");
      addKeyUpEventListener(component, listener);
      DomGlobal.console.debug("Added native keyup listener to {" + component + "}");
  ...
  private static void checkWidgetHasTextOrValue(final Object component) {
    if (component instanceof Widget
        && !(component instanceof HasText || component instanceof TakesValue)) {
      throw new RuntimeException("Widget must implement either " + TakesValue.class.getName()
          + " or " + HasText.class.getName() + "!");
    }
  }

  private static <K, V> Supplier<Map<K, V>> mergeToLeft(final Supplier<Map<K, V>> f,
      final Supplier<Map<K, V>> g) {
    return () -> {
  ...
  /**
   * Binds the provided component to the specified property (or property chain) of the model
   * instance associated with this proxy (see {@link DataBinder#setModel(Object, StateSync)}).
   * 
   * @param component the UI component to bind to, must not be null.
   * @param property the property of the model to bind the widget to, must not be null.
   * @param converter the converter to use for this binding, null if default conversion should be
   *        used. See {@link Convert#getConverter(Class, Class)} and
   ...
   * @param component the UI component to bind to, must not be null.
   * @param property the property of the model to bind the widget to, must not be null.
   * @param providedConverter the converter to use for this binding, null if default conversion
   *        should be used. See {@link Convert#getConverter(Class, Class)} and
   *        {@link Convert#identityConverter(Class)} for possible arguments.
   * @param bindOnKeyUp a flag indicating that the property should be updated when the widget fires
   *        a {@link KeyUpEvent} along with the default {@link ValueChangeEvent}.
   * @param initialState specifies whether to use the model or component state when initially
   *        synchronizing this binding. If null, the values will not be synchronized until a change
   *        is made to the model or component.
   * 
   * @return binding the created binding.
   */
  public Binding bind(final Object component, final String property,
    ...
    DomGlobal.console.debug("Using converter: {" + converter.getModelType().getSimpleName()
        + "} <--> {" + converter.getComponentType().getSimpleName() + "}");
    ...
    final Function<BindableProxyAgent<?>, Supplier<Map<Class<? extends org.gwtproject.event.shared.Event>, HandlerRegistration>>> registrar =
        agent -> addHandlers(component, bindOnKeyUp, uiGetter,
            modelUpdater(component, converter, lastSubProperty, agent));
  ...
  private Consumer<Object> modelUpdater(final Object component, final Converter converter,
      final String lastSubProperty, final BindableProxyAgent<?> agent) {
    return uiValue -> {
  ...
  private Binding bindHelper(final Object component, final String property,
      final Converter converter,
      final Function<BindableProxyAgent<?>, Supplier<Map<Class<? extends org.gwtproject.event.shared.Event>, HandlerRegistration>>> registrar,
    ...
    if (property.contains(".")) {
      ...
      return bindDirectProperty(component, property, converter, registrar.apply(this), uiGetter,
          initialState);
    }
  }

  private Binding bindNestedProperty(final Object component, final String property,
      final Converter<?, ?> converter,
      final Function<BindableProxyAgent<?>, Supplier<Map<Class<? extends org.gwtproject.event.shared.Event>, HandlerRegistration>>> registrar,
      final Optional<Supplier<Object>> uiGetter, final StateSync initialState) {
    ...
    final BindableProxyAgent<?> nestedAgent =
        ((BindableProxy<?>) nestedBinder.getModel()).getBindableProxyAgent();
    nestedBinder.addBinding(subProperty, nestedAgent.bindHelper(component, subProperty, converter,
        registrar, uiGetter, initialState));
  ...
  private void trySettingModelProperty(final String property, final Converter converter,
      final Object uiValue, final Object newValue) {
    try {
      proxy.set(property, newValue);
    } catch (final Throwable t) {
      if (newValue == null && isCausedByNullOrUndefined(t)) {
        /*
         * Don't fail here because likely this is an error from trying to unbox a null value to a
         * primitive. XXX Maybe we should check for boxed types before trying to set the property.
         */
        DomGlobal.console.warn("Encountered a null while trying to set the property [" + property
            + "] to [" + newValue + "].", t);
      } else {
        throw new RuntimeException(
            "Error while setting property [" + property + "] to [" + newValue + "] converted from ["
                + uiValue + "] with converter [" + converter.getComponentType().getName() + " -> "
                + converter.getModelType().getName() + "].",
            t);
      }
    }
  }
  ...
  private Binding bindDirectProperty(final Object component, final String property,
      final Converter converter,
      final Supplier<Map<Class<? extends org.gwtproject.event.shared.Event>, HandlerRegistration>> handlerRegistrar,
      final Optional<Supplier<Object>> uiGetter, final StateSync initialState) {
  ...
  private Binding createBinding(final Object component, final String property,
      final Converter converter,
      final Supplier<Map<Class<? extends org.gwtproject.event.shared.Event>, HandlerRegistration>> handlerRegistrar) {
    ...
    for (final Binding binding : bindings.values()) {
      if (binding.getComponent().equals(component) && !property.equals(binding.getProperty())) {
        throw new ComponentAlreadyBoundException(
            "Widget already bound to property: " + binding.getProperty());
      }
    }
  }

  private Converter findConverter(final String property, final Class<?> propertyType,
      final Object component, final Converter userProvidedConverter) {
    final Optional<Class<?>> componentValueType;

    if (component instanceof Widget) {
      final Class<?> presumedType =
          (userProvidedConverter == null) ? propertyType : userProvidedConverter.getComponentType();
      componentValueType =
          Optional.ofNullable(Convert.inferWidgetValueType((Widget) component, presumedType));
    } else if (isElement(component)) {
      final Element element = BoundUtil.asElement(component);
      if (InputElement.is(element)) {
        componentValueType = Optional.ofNullable(
            ElementWrapperWidget.getValueClassForInputType(element.getPropertyString("type")));
    ...
    if (userProvidedConverter != null) {
      ...
      componentValueType.ifPresent(
          t -> validateTypes(property, t, userProvidedConverter.getComponentType(), "widget"));
      ...
      if (converter == null) {
        throw new RuntimeException("Cannot convert between " + propertyType.getName() + " and "
            + effectiveComponentType.getName() + " for property [" + property + "] in "
  ...
  private void validateTypes(final String property, final Class<?> actualType,
      final Class<?> converterType, final String modelOrWidget) {
  ...
  private boolean oneTypeIsInterface(final Class<?> propertyType,
      final Class<?> converterModelType) {
   ...
   * 
   * @param property the property of the model to bind the widget to, must not be null. The property
   *        must be of a @Bindable type.
   */
  private DataBinder createNestedBinder(final String property, final StateSync initialState) {
    ...
    if (binder == null) {
      ...
      for (final PropertyChangeHandler<?> handler : propertyChangeHandlerSupport.specificPropertyHandlers
          .get("**")) {
    ...
    if (property.indexOf('.') != property.lastIndexOf('.')) {
      ((BindableProxy<?>) binder.getModel()).getBindableProxyAgent()
          .createNestedBinder(property.substring(property.indexOf('.') + 1), initialState);
    }
  ...
  private void validatePropertyExpr(final String property) {
    ...
    if (property.contains("*.")) {
      throw new InvalidPropertyExpressionException(
          "Wildcards can only appear at the end of property expressions : " + property);
    }
  }
   ...
   * 
   * @param binding the name of the model property to unbind, must not be null.
   */
  public void unbind(final Binding binding) {
    ...
    if (dotPos > 0) {
      ...
      if (binder != null) {
        final BindableProxyAgent<T> nestedAgent =
            ((BindableProxy<T>) binder.getModel()).getBindableProxyAgent();
        final Collection<Binding> nestedBindings =
            nestedAgent.bindings.get(property.substring(dotPos + 1));
        for (final Binding nestedBinding : nestedBindings
            .toArray(new Binding[nestedBindings.size()])) {
   ...
   * 
   * @param <P> The property type of the changed property.
   * @param sync True if a {@link BindableListChangeHandler} component bound to a list should have
   *        it's value set via {@link TakesValue#setValue(Object)}.
   * @param property The name of the property that changed. Must not be null.
   * @param oldValue The old value of the property.
   * @param newValue The new value of the property.
   */
  public <P> void updateWidgetsAndFireEvent(final boolean sync, final String property,
      final P oldValue, final P newValue) {
   ...
   * @param oldValue The old value of the property.
   * @param newValue The new value of the property.
   * @param excluding A widget reference that does not need to be updated (the origin of the value
   *        change event).
   */
  private <P> void updateWidgetsAndFireEvent(final boolean sync, final String property,
      final P oldValue, final P newValue, final Object excluding) {
    ...
    for (final Binding binding : bindings.get(property)) {
      ...
      } else if (component instanceof IsElement || component instanceof IsElement
          || isElement(component)) {
  ...
  private <P> void updateComponentValue(final P newValue, final HasText component,
      final Converter converter) {
  ...
  private <P> void updateComponentValue(final P newValue, final TakesValue component,
      final Converter converter) {
    final Object widgetValue = converter.toWidgetValue(newValue);
  ...
  /**
   * Fires a property change event unless the property is {@code "this"}. The {@code "this"}
   * property is only fired for types with no other properties.
   * 
   * @param <P> The property type of the changed property.
   * @param property The name of the property that changed. Must not be null.
   * @param oldValue The old value of the property.
   ...
   */
  private <P> boolean maybeFirePropertyChangeEvent(final String property, final P oldValue,
      final P newValue) {
    knownValues.put(property, newValue);

    final PropertyChangeEvent<P> event =
        new PropertyChangeEvent<>(proxy, Assert.notNull(property), oldValue, newValue);
   ...
   * provided {@link StateSync}.
   * 
   * @param component The widget to synchronize. Must not be null.
   * @param property The name of the model property that should be synchronized. Must not be null.
   * @param converter The converter specified for the property binding. Must not be null.
   * @param uiGetter An optional function for getting the value from the UI component. Must not be
   *        null.
   * @param initialState If {@link StateSync#FROM_MODEL} the UI value will be updated from the
   *        model. If {@link StateSync#FROM_UI} the model value will be updated from the UI. If null
  ...
  private void syncState(final Object component, final String property, final Converter converter,
    ...
    if (initialState != null) {
      ...
      final Object value =
          uiValue.map(v -> initialState.getInitialValue(modelValue, v)).orElse(modelValue);
   ...
   * 
   * @param property the name of the list property
   * 
   ...
   * 
   * @param property the name of the list property
   * @param list the list that needs to be proxied
   * 
   * @return a new the wrapped (proxied) list or the provided list if already proxied
   */
  ...
  private void addHandlersForBindableListWrapper(final String property,
      final BindableListWrapper newList) {
    ...
    for (final Binding binding : bindings.get(property)) {
      if (binding.getComponent() instanceof BindableListChangeHandler) {
        modelChangeHandlers
            .add(newList.addChangeHandler((BindableListChangeHandler) binding.getComponent()));
      }
    }
  }
  ...
  @Override
  public <P> PropertyChangeUnsubscribeHandle addPropertyChangeHandler(final String property,
      final PropertyChangeHandler<P> handler) {
    validatePropertyExpr(property);
   ...
   * 
   * @param pchs the instance who's change handlers will be merged, must not be null.
   */
   ...
   * 
   * @param other the agent to compare against.
   * @param initialState If {@link StateSync#FROM_MODEL} the state from this agent's model overrides
   *        the other. If {@link StateSync#FROM_UI} the state from the other agent's model overrides
   *        this one. If null, default to {@link StateSync#FROM_MODEL}.
   */
  public void fireChangeEvents(final BindableProxyAgent other, final StateSync initialState) {
    for (final String property : propertyTypes.keySet()) {
      final Object curValue, oldValue, thisValue = knownValues.get(property),
      ...
      if ((curValue == null && oldValue != null)
        ...
        if (nestedBinder != null && otherNestedBinder != null) {
          final BindableProxyAgent nestedAgent =
              ((BindableProxy<T>) nestedBinder.getModel()).getBindableProxyAgent();
          final BindableProxyAgent otherNestedAgent =
              ((BindableProxy<T>) otherNestedBinder.getModel()).getBindableProxyAgent();
