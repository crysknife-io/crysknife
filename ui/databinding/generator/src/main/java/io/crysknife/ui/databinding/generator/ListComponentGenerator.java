...
public class ListComponentGenerator extends BeanIOCGenerator {
   ...
   * 
   * if (fieldPoint.getType().getTypeParameters().size() != 2) { throw new
   * Error(ListComponent.class.getCanonicalName() + " must be typed"); }
   * 
   * TypeMirror type1 = fieldPoint.getType().getTypeParameters().get(0).asType(); TypeMirror type2 =
   * fieldPoint.getType().getTypeParameters().get(1).asType();
   * 
   * 
   * ClassOrInterfaceType classOrInterfaceType = new ClassOrInterfaceType();
   * classOrInterfaceType.setName(DefaultListComponent.class.getSimpleName());
   * classOrInterfaceType.setTypeArguments(new ClassOrInterfaceType().setName(type1.toString()), new
   * ClassOrInterfaceType().setName(type1.toString()));
   * 
   * 
   * 
   ...
   * ExpressionStmt(new ObjectCreationExpr().setType(classOrInterfaceType)));
   * 
